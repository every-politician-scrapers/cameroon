...
module.exports = function () {
  ...
  return `SELECT DISTINCT ?item ?name ?region ?regionLabel
    ...
    WHERE {
      ...
      OPTIONAL {
        ?ps pq:P768 ?region .
        OPTIONAL { ?region rdfs:label ?regionLabel FILTER (LANG(?regionLabel)="fr") }
      }

      OPTIONAL {
        ?ps prov:wasDerivedFrom ?ref .
        ?ref pr:P854 ?source FILTER CONTAINS(STR(?source), 'www.crtv.cm') .
